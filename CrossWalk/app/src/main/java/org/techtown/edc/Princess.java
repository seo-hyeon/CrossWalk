...
public class Princess implements Serializable {
    ...
    //(갈색 약장~)26울새의 깃털, 27 나비의 날개, 28 고양이의 수염, 29 송어의 비늘, 30 개구리 알
    ...
    private int f3_choices[] = new int[50];
    private int end[] = new int[10];
    private int location; //1층 0 : 새장, 1 : 큰문, 2 : 조리대, 3: 창문, 4 : 식탁, 5 : 계단
    ...
    public void setF3_choices(int c) {this.f3_choices[c] = 1;}
    public Boolean getF3_choices(int c) {
        if(this.f3_choices[c] == 1){
            return true;
        } else{
            return false;
        }
